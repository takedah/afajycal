...
class TestMatchScheduleAction(unittest.TestCase):
        ...
        self.html_content = [
            {
                ...
                "kickoff_time": datetime(self.year, 6, 2, 14, 0, tzinfo=self.JST),
                "home_team": "六合",
                "away_team": "中富良野",
                "studium": "花咲球技場",
            },
            {
                ...
                "kickoff_time": datetime(self.year, 6, 8, 14, 0, tzinfo=self.JST),
                "home_team": "永山南",
                "away_team": "六合",
                "studium": "花咲球技場",
            },
            {
                ...
                "kickoff_time": datetime(self.year, 1, 1, 0, 0, tzinfo=self.JST),
                "home_team": "test",
                "away_team": "test",
                "studium": "test",
            },
    ...
    def test_get_number_of_matches(self, config_mock):
        ...
        self.assertEqual(
            MatchScheduleAction.get_number_of_matches(
                "六合", datetime(2019, 6, 2, 13, 0, tzinfo=self.JST)
            ),
            2,
        )
    ...
    def test_find(self, config_mock):
        ...
        res = MatchScheduleAction.find(
            "六合", "サテライト", datetime(2019, 6, 2, 13, 0, tzinfo=self.JST)
        )
        schedule = res[0]
        number = res[1]
        ...
        self.assertEqual(
            schedule[0].kickoff_time, datetime(2019, 6, 2, 14, 0, tzinfo=self.JST)
        )
        self.assertEqual(schedule[1].home_team, "永山南")
        self.assertEqual(schedule[1].studium, "花咲球技場")
        self.assertEqual(
            schedule[1].kickoff_time, datetime(2019, 6, 8, 14, 0, tzinfo=self.JST)
        )
        self.assertEqual(number, 2)
        res = MatchScheduleAction.find(
            "六合", "サテライト", datetime(2019, 9, 18, 9, 0, tzinfo=self.JST)
        )
        schedule = res[0]
        number = res[1]

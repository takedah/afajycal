...
def categories():
    ...
    for category in categories:
        tmp.append([urllib.parse.quote(category), category])
...
def find():
    ...
    results_number = res[1]
    last_update = MatchScheduleAction.get_last_update()
    title = '"' + team_name + '"' + " " + '"' + category + '"' + " " + "の試合スケジュール検索結果"
    return render_template(
        "find.html",
        title=title,
        this_year=this_year,
...
def not_found(error):
    ...
    return render_template("404.html", title=title, categories=categories())

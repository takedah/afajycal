...
def index():
    ...
    for team_name in tmp:
        all_team_names.append([team_name, MatchScheduleAction.get_number_of_matches(team_name)])
...
def find_all():
    ...
    last_update = MatchScheduleAction.get_last_update_time()
    title = 'Search results of ' + '"' + team_name + '"' + ' "' + category + '"' + ' match schedules'
    return render_template(
        ...
        results_number=results_number, next_game=next_game, last_update=last_update)
